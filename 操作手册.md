...
# 安装核心依赖
pip install onnxruntime-gpu insightface opencv-python numpy ffmpeg-python tqdm rich pyyaml
...
```bash
# 创建项目目录结构
mkdir -p dlc_batch/{engine,tracker,io,ui,config,utils}
mkdir -p models videos_in outputs assets
touch dlc_batch/__init__.py
touch dlc_batch/cli.py
